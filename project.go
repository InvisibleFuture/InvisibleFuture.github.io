...
func (p Project) Create(name, master string) {
	...
	if err != nil {
		panic("PROJECT_TIME_DB CREATE ERROR")
	}

	err = PROJECT_MASTER_DB.Put([]byte(p.Id), []byte(master), nil)
	if err != nil {
		panic("PROJECT_MASTER_DB CREATE ERROR")
	}
...
func (l List) GetProjects() ([]ListProject, error) {
	...
	for _, id := range ids {
		...
		if err != nil {
			panic("PROJECT_NAME_DB GET ERROR")
		}
